...
class MancalaGame:
    ...
    def player_move(self, index):
        if self.player_turn and self.board[index] > 0:
            ...
            if self.check_game_over():
                self.end_game()
            elif not self.has_valid_moves(is_player=False):
                self.player_turn = True
                self.master.after(1000, self.player_move, index)
            elif last_pit == 6:  # Player gets an extra turn
                self.player_turn = True
            else:
    ...
    def ai_move(self):
        if not self.player_turn:
            ...
            if self.check_game_over():
                self.end_game()
            elif not self.has_valid_moves(is_player=True):
                self.player_turn = False
                self.master.after(1000, self.ai_move)
            elif last_pit == 13:  # AI gets an extra turn
                self.player_turn = False
                self.master.after(1000, self.ai_move)
            else:
                self.player_turn = True
    ...
    def has_valid_moves(self, is_player):
        start = 0 if is_player else 7
        end = 6 if is_player else 13
        return any(self.board[i] > 0 for i in range(start, end))

    def check_game_over(self):
        return not self.has_valid_moves(True) and not self.has_valid_moves(False)
