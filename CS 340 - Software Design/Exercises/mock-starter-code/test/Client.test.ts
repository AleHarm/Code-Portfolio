...
describe("m4 integrated test", () => {
   ...
   */
  beforeEach(() => {
    // DONE: Mock the Service class here instead of calling new
    mockService = mock(Service);

    // DONE: Create the mockServiceInstance as an instance of the mockService
    mockServiceInstance = instance(mockService);
    client = new Client();

    // DONE: Create the spy for the client.
    clientSpy:Client = spy(Client);
    

    // DONE: Create the spyClientInstance as an instance of the client spy
    spyClientInstance = instance(clientSpy);

    // DONE: Use the ts-mokito when/thenReturn to tell the spyClient (not the client) to return the instance of the mock service when the factory method is called
    when(spyClientInstance.makeService()).thenReturn(mockServiceInstance);
  });
  ...
  it("testConvertValue", () => {
    ...
    // DONE: Use ts-mockito when/thenReturn to tell the mock service (not the instance of the mock service) to return 2 for input 35 when it's getDecimalDigitCount(int) method is called
    when(mockService.getDecimalDigitCount(35)).thenReturn(2);

    // DONE: Change this to use the spyClientInstance
  ...
  it("testCreateFormattedStringsWithAnswer", () => {
    ...
    when(mockService.processList(anything())).thenCall((parameter: string[]) =>{ return parameter != null}));
    ...
    // DONE: use ts-mockito verify to ensure that mockService.processList is called
    verify(mockService.processList(anything())).atLeast(1);
  });
});
